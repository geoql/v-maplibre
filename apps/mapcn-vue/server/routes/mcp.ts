...
export default defineEventHandler(async (event: H3Event) => {
  ...
  for (const message of messages) {
    ...
    if (message.method === 'initialize') {
      responses.push(
        success(id, {
          ...
          instructions:
            'mapcn-vue map component registry. Use list_components to browse or search the MapLibre, deck.gl, and LiDAR map components, get_component for full source and install instructions, and get_install_command for the shadcn-vue CLI command.',
        }),
      );
      continue;
    }
    ...
    if (message.method === 'tools/call') {
      ...
      try {
        result = callMcpTool(toolName, args);
      } catch {
        result = {
