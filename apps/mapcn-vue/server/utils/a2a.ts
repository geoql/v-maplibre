import type {
  ...
  A2aMessage,
  A2aTask,
} from '~~/apps/mapcn-vue/server/types/a2a';
import type { RegistryIndexItem } from '~~/apps/mapcn-vue/server/types/mcp';

const SITE_URL = 'https://mapcn-vue.geoql.in';

export const A2A_AGENT_CARD: A2aAgentCard = {
  name: 'mapcn-vue Component Finder',
  description:
    'Deterministic agent for the mapcn-vue registry of Vue map components built on MapLibre GL, deck.gl, and LiDAR. Send a plain-text description of the map layer you need (e.g. "animated trips layer" or "lidar point cloud viewer") and it returns matching components with descriptions, docs links, and shadcn-vue CLI install commands. Backed by registry search, not an LLM: responses are instant, repeatable, and free of hallucination.',
  version: '1.0.0',
  supportedInterfaces: [
    {
      url: `${SITE_URL}/a2a`,
  ...
  provider: { organization: 'mapcn-vue', url: SITE_URL },
  documentationUrl: `${SITE_URL}/docs/components`,
  capabilities: {
    streaming: false,
  ...
  skills: [
    {
      id: 'find-components',
      name: 'Find Vue map components',
      description:
        'Search the mapcn-vue registry by keywords or a short description. Returns matching MapLibre / deck.gl / LiDAR map components with title, description, docs URL, and the shadcn-vue CLI install command.',
      tags: ['vue', 'nuxt', 'maplibre', 'deck.gl', 'lidar', 'maps', 'search'],
      examples: [
        'deck.gl heatmap layer',
        'lidar point cloud viewer',
        'animated trips visualization',
      ],
...
function formatResults(query: string, items: RegistryIndexItem[]): string {
  if (items.length === 0) {
    return [
      `No mapcn-vue components match "${query}".`,
      `Browse the full catalog at ${SITE_URL}/docs/components or query the MCP server at ${SITE_URL}/mcp (list_components).`,
    ].join('\n');
  }
  return [
    `Found ${items.length} mapcn-vue component(s) matching "${query}":`,
    ...items.map((item) =>
      [
    ...
    return {
      ...
      status: {
        ...
        message: {
          ...
          parts: [
            {
              text: 'Describe the map component you are looking for, e.g. "deck.gl heatmap" or "lidar point cloud viewer".',
            },
          ],
