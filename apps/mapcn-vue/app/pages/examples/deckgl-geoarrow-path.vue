  ...
  <div class="flex flex-col lg:flex-row gap-3">
    ...
    >
      <div class="flex items-center justify-between">
        <h2 class="text-sm font-semibold text-foreground">GEOARROW LINES</h2>
      ...
      <div v-else class="text-xs text-muted-foreground">
        {{ table?.numRows }} routes · GeoArrow IPC · LineString
      </div>

      <div class="flex flex-col gap-2">
        <label class="text-xs font-medium text-foreground">Line width</label>
        <Slider v-model="widthScale" :min="1" :max="50" :step="1" />
